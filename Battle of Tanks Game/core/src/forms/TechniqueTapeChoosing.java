...
public class TechniqueTapeChoosing implements Disposable {
	...
	private StoreTechniqueItem itemChecked;
	
	public TechniqueTapeChoosing () {
		initBackgrounds();
		initTechniques();
		initTape();
		initButtons();
	...
	private void initBackgrounds () {
		...
		background.setPosition(100, CameraController.getInstance().getHeight() - 500);
	...
	public void show (SpriteBatch batch) {
		...
		for (StoreTechniqueItem item : items) {
			...
			if (item.isReleased()) {
				if (itemChecked != null) {
					itemChecked.setTexture(storeItemTexture);
					itemChecked.setTextureOver(storeItemOverTexture);
				}
				itemChecked = item;
