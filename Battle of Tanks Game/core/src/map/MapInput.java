...
public class MapInput {
	public static void loadMap (String path, ChunkGroup group, Array <MapTexture> mapTextures, Array <MapTexture> wallTextures, Array <Spawn> spawnCommandOne, Array <Spawn> spawnCommandTwo, Graph graph) {
			...
			for (index = 8; index < cmd.length; index++) {
				if (cmd[index].equals("mapTexturesBegin")) {
					...
					while (!next.equals("mapTexturesEnd")) {
						MapTexture mapTexture = new MapTexture(Integer.parseInt(next), null, path + "/images/" + Integer.parseInt(next) + ".png");
					...
					while (!next.equals("wallTexturesEnd")) {
						...
						MapTexture wallTexture = new MapTexture(Integer.parseInt(next), null, tempPath);
