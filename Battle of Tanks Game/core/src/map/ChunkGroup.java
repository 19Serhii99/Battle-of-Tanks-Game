...
public class ChunkGroup {
	...
	public void loadMap (final String path, final Array <MapTexture> mapTextures, final Array <MapTexture> wallTextures, final Array <Spawn> spawnCommandOne, final Array <Spawn> spawnCommandTwo, final Graph graph) {
		final ChunkGroup group = this;
		new Thread (new Runnable () {
			@Override
			public void run() {
				MapInput.loadMap(path, group, mapTextures, wallTextures, spawnCommandOne, spawnCommandTwo, graph);
				lookingForLastChunks();
				graph.aStar(graph.getVertices().get(0), graph.getVertices().get(20));
				isLoaded = true;
			}
		}).start();
	}
