...
public class Mathematics {
	...
	public static Vector2 getIntersectionPoint (float x1, float y1, float x2, float y2, float x3, float y3, float x4, float y4) {
		...
		final float differenceLeftX = x2 - x1;
		final float differenceRightX = x4 - x3;
		final float differenceLeftY = y2 - y1;
		final float differenceRightY = y4 - y3;
		...
		if (differenceLeftX == 0.0f || differenceRightX == 0.0f) {
			...
			final float leftC = - x1 * differenceLeftY + differenceLeftX * y1;
			final float rightC = -x3 * differenceRightY + differenceRightX * y3;
			final float a = differenceLeftX * differenceRightY;
			final float b = - leftC * differenceRightY + rightC * differenceLeftY;
			final float c = - differenceRightX * differenceLeftY;
			final float d = a + c;
			y = - b / d;
	...
	public static boolean isCollision (float x1, float y1, float x2, float y2, float x3, float y3, float x4, float y4, GeometricData data, float pointX, float pointY) {
		final float vector1 = getDistance(x1, y1, pointX, pointY);
		final float vector2 = getDistance(x2, y2, pointX, pointY);
		final float vector3 = getDistance(x3, y3, pointX, pointY);
		final float vector4 = getDistance(x4, y4, pointX, pointY);
		final float area1 = getTriangleArea(vector1, vector2, data.getLeftEdge());
		final float area2 = getTriangleArea(vector2, vector3, data.getTopEdge());
		final float area3 = getTriangleArea(vector1, vector3, data.getDiagonal());
		if (area1 + area2 + area3 <= data.getArea() + 1) {
			return true;
		} else {
			final float area4 = getTriangleArea(vector1, vector4, data.getBottomEdge());
			final float area5 =  getTriangleArea(vector4, vector3, data.getRightEdge());
